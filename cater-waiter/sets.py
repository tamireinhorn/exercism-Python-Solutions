...
def categorize_dish(dish_name, dish_ingredients):
    ...
    possibilities = {'VEGAN': VEGAN, 'VEGETARIAN': VEGETARIAN, 'PALEO': PALEO, 'KETO': KETO, 'OMNIVORE': OMNIVORE}
    for diet in possibilities: 
        diet_set = possibilities.get(diet)
        if ingredient_set.issubset(diet_set):
...
def tag_special_ingredients(dish):
    ...
    return (f'{dish_name}', ingredient_set.intersection(SPECIAL_INGREDIENTS))
...
def compile_ingredients(dishes):
    ...
    """
    return set().union(*dishes) #* unpacks the list, giving us everything in a single iterable.
